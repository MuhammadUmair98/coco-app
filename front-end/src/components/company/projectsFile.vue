<template>
    <div class="relative overflow-x-auto my-20">
        ...
        <button :disabled="file == ''" @click="uploadFile()" class="bg-blue-500 text-white px-4 py-2 rounded mx-2 my-2"  v-if="userRole === 'Admin'">
            Save File
        </button>
            ...
            <tbody>
                <tr v-for="file in projectFiles" :key="file.id"
                    ...
                    <td class="px-6 py-4">
                        ...
                        <button @click="downloadFile(file.file_url)" class="bg-blue-500 text-white px-4 py-2 rounded mx-2"  v-if="userRole === 'Admin'">
                            Download File
                        </button>
                    </td>
                </tr>
...
export default {
    ...
    methods: {
        async fetchProject() {
            try {
                const response = await axios.get(`http://localhost:3000/api/projects/${this.$route.params.id}`);
