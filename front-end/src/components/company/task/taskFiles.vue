<template>
    <div class="relative overflow-x-auto my-20">
        ...
        <div class="flex items-center justify-center w-full"  v-if="userRole != 'Admin'">
        ...
        <button  v-if="userRole != 'Admin'" :disabled="file == ''" @click="uploadFile()" class="bg-blue-500 text-white px-4 py-2 rounded mx-2 my-2">
            Save File
        </button>
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                <tr v-for="file in files" :key="file.id"
                    ...
                    <td class="px-6 py-4">
                        <button @click="deleteFile(file.id)" class="bg-blue-500 text-white px-4 py-2 rounded"  v-if="userRole != 'Admin'">
                            Delete
                        </button>
...
export default {
    ...
    methods: {
        async fetchFiles() {
            try {
                const response = await axios.post(`http://localhost:3000/api/projects/task-subtask-files`, { id: this.$route.params.id, identifier: 'task' });
        ...
        async uploadFile() {
            try {
                ...
                formData.append('identifier', 'task');
