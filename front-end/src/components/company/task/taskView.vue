        ...
        <div>
            ...
            <div class="relative overflow-x-auto">
                <div class="flex justify-between items-center p-4">
                    ...
                    <button @click="addProjectTask" class="bg-blue-500 text-white px-4 py-2 rounded">
                        Add Task
                    </button>
                </div>
                <table class="w-full text-xs md:text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        <tr v-for="project in projectsTaskSubTask" :key="project.proj_id"
                            ...
                            <td class="px-6 py-4">
                                <button @click="viewSubTasks(project.task_id)" class="text-xs text-red-500 border border-red-500 px-2 py-1 rounded">
                                    View SubTasks
                                </button>
                                <button @click="addSubTaskView(project.task_id)" class="text-xs text-red-500 border border-red-500 px-2 py-1 rounded my-2">
                                    Add SubTasks
                                </button>
        ...
        <div>
            ...
            <div class="relative overflow-x-auto">
                ...
                <table class="w-full text-xs md:text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        <tr v-for="project in subTasks" :key="project.proj_id"
                            class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            <td class="px-6 py-4">
...
export default {
    ...
    methods: {
        async fetchProjectsTask() {
            try {
                const response = await axios.get(`http://localhost:3000/api/projects/project-tasks-sub-tasks/${this.$route.params.id}`);
        ...
        addProjectTask: function () {
            this.$router.push({ name: 'addTask', params: { id: this.$route.params.id } });
        },
